...
def generate_path(viewpoint_cameras, n_frames=480):
  ...
  for c2w in new_poses:
      ...
      if hasattr(cam, "R"):
          cam.R = c2w[:3, :3].astype(np.float32)
      if hasattr(cam, "T"):
          cam.T = w2c[:3, 3].astype(np.float32)
...
def save_img_u8(img, pth, srgb=False):
  """Save an image (probably RGB) in [0, 1] to disk as a uint8 PNG in sRGB colorspace."""
  with open(pth, 'wb') as f:
    if srgb:
      img = linear_to_srgb(img)
    img = np.clip(np.nan_to_num(img), 0., 1.)
    Image.fromarray((img * 255.).astype(np.uint8)).save(f, 'PNG')
